...
pub struct SignatureGenerator {
    ...
    fft_outputs: Vec<Vec<f32>>, //[[f32; 1025]; 256],
    ...
    spread_fft_outputs: Vec<Vec<f32>>, //[[f32; 1025]; 256],
    /// Ring buffer.
...
impl SignatureGenerator {
    ...
    pub fn make_signature_from_f32_buffer(
        f32_mono_buffer: Vec<f32>,
        sample_rate: u32,
    ) -> DecodedSignature {
        let t_sample_rate = 16000u32;
        let di = sample_rate.div(t_sample_rate) as usize;

        let mut s16_mono_16khz_buffer = Vec::<i16>::new();

        for i in (0..f32_mono_buffer.len()).step_by(di) {
            s16_mono_16khz_buffer.push((f32_mono_buffer[i] * 32767f32) as i16);
        }
    ...
    pub fn make_signature_from_buffer(s16_mono_16khz_buffer: Vec<i16>) -> DecodedSignature {
        ...
        for _ in 0..256 {
            spread_fft_outputs.push([0.0; 1025].iter().cloned().collect());
            fft_outputs.push([0.0; 1025].iter().cloned().collect());
        }
        let mut this = SignatureGenerator {
            ring_buffer_of_samples: [0; 2048].iter().cloned().collect(),
            ring_buffer_of_samples_index: 0,

            reordered_ring_buffer_of_samples: [0.0; 2048].iter().cloned().collect(),
    ...
    fn do_fft(self: &mut Self, s16_mono_16khz_buffer: &[i16]) {
        ...
        let multipliers = hanning_window_2048_multipliers();
        for index in 0..=2047 {
